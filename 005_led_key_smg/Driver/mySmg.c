...
void smg_proc()
{
    ...
    switch(cj.state)                                // 根据状态选择不同的显示格式
    {
        case 10:
            if(f_wei)
                sprintf(dis_str,"F%02u-%02ubb",cj.m,cj.s); // 格式化字符串，显示频率和时间
            else
                sprintf(dis_str,"   -%02ubb",cj.s);         // 格式化字符串，显示时间
            break;
		break;
        case 20:
            if(f_wei)
                sprintf(dis_str,"d    %3u",cj.key);          // 格式化字符串，显示按键值
            else
                sprintf(dis_str,"E  %5u",cj.freq);           // 格式化字符串，显示频率
            break;
        case 30:sprintf(dis_str,"b%3.1f %3u",cj.t,cj.d);break; // 格式化字符串，显示温度和距离
        case 40:sprintf(dis_str,"A    %3u",cj.ad);break;     // 格式化字符串，显示AD值
    }
